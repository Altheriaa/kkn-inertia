...
const form = useForm({
    jadwal_kkn_id: props.kelompok.jadwal_kkn_id,
    dpl_id: props.kelompok.dpl_id,
    lokasi_kkn_id: props.kelompok.lokasi_kkn_id,
    nama_kelompok: props.kelompok.nama_kelompok,
    jenis_kkn: props.kelompok.jenis_kkn,
});

// Picu react select dosen
watch(() => form.jadwal_kkn_id,  (newId) => {
    // jangan reset kalo sama dengan jadwal yang sedang di edit
    if (newId === props.kelompok.jadwal_kkn_id) return;

    // Reset isi form jika jadwal diubah
    form.dpl_id = ''; 
    form.lokasi_kkn_id = ''; 

    router.get(`/admin/plotting/${props.kelompok.id}/edit`, 
        { jadwal_kkn_id: newId }, 
        { 
...
const submit = () => {
    form.put(`/admin/plotting/${props.kelompok.id}`, {
        preserveScroll: true,
    });
};
</script>
                        ...
                        <div class="card-header p-0 position-relative mt-n4 mx-3 z-index-2">
                            <div class="bg-gradient-dark shadow-dark border-radius-lg pt-4 pb-3 d-flex align-items-center">
                                <div class="d-flex align-items-center ps-3">
                                    ...
                                    <div>
                                        <h4 class="text-white text-capitalize mb-0">Edit Kelompok KKN</h4>
                                        <p class="text-white text-sm mb-0 opacity-8">Lengkapi data kelompok kkn</p>
                                    </div>
                                </div>
                            </div>
                        </div>
